...
export default function ItemCart({item, isFirstItem}) {
  const [hasStock, setHasStock] = useState(item.stock > 0);


  return (
    <article className={clsx("w-full flex gap-4 border-b border-gray-200 p-4", isFirstItem && "border-t")}>
      ...
      <section className="flex flex-col justify-between w-full">
        <article className="grid grid-cols-1 md:grid-cols-3 gap-2">
          <div className="grid grid-cols-1 gap-2">
              <h2 className="text-lg font-semibold font-biorhyme">{item.title}</h2>
        ...
        <article className="flex gap-2 items-center">
          <span>{hasStock ? <Check size={16} color="green"/> : <X size={16} color="red"/>}</span>
